...
$alerts_sql = "SELECT * FROM alerts ORDER BY date DESC";
...
<head>
    ...
    <title>Alerts - Forest & Wildlife Monitoring Dashboard</title>
...
<body class="text-white relative min-h-screen">
    ...
    <main class="relative z-20 container mx-auto pt-24 pb-20">
        <div class="glass p-6 md:p-10 rounded-xl">
            <h2 class="text-3xl md:text-4xl font-bold bg-gradient-to-r from-green-400 to-blue-500 bg-clip-text text-transparent mb-6 md:mb-8">Alerts & Notifications</h2>
            <div class="mb-6">
                <select class="p-3 glass border-none rounded-full text-gray-400">
                    <option>Filter by Location</option>
                    <!-- Add dynamic locations via PHP if needed -->
                </select>
            </div>
            <div class="space-y-6">
                <?php while ($alert = $alerts_result->fetch_assoc()): ?>
                    <div class="glass p-4 rounded-lg">
                        <h3 class="text-xl md:text-2xl font-bold"><?php echo $alert["title"]; ?></h3>
                        <p class="text-sm md:text-base text-gray-300"><?php echo $alert["location"]; ?> | <?php echo $alert["type"]; ?> | <?php echo $alert["date"]; ?></p>
                        <p class="mt-2"><?php echo $alert["description"]; ?></p>
                    </div>
                <?php endwhile; ?>
            </div>
        </div>
